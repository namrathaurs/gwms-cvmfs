...
prep_cvmfs_env () {
	...
	if [[ $1 == "MOUNT" ]]; then
		tar -xvzf glidein_cvmfs.tar.gz
		./mycvmfsexec -- ls -al
	#elif [ $1 == "UNMOUNT" ]; then
		# do .cvmfsexec and mycvmfsexec files need to be removed upon unmount of CVMFS????
	fi

	#echo $(dirname $(readlink -f $0))	
	export THIS_SCRIPT_DIR=$(dirname $(readlink -f $1))
	export CVMFSEXEC_HOME=$THIS_SCRIPT_DIR/.cvmfsexec
...
mount_cvmfs_repos () {
	...
	prep_cvmfs_env "MOUNT"	
	
	#echo $CVMFSEXEC_HOME	
	$CVMFSEXEC_HOME/mountrepo $1
